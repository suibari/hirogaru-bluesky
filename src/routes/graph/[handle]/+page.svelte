<script>
  ...
  import { page } from '$app/stores'; // URLパラメータを取得するために追加
  // external modules
  ...
  // ページレンダリング後の処理
  onMount(() => {
    // URLからhandleパラメータを取得し、inputHandleにセット
    const urlHandle = $page.params.handle;
    if (urlHandle) {
      inputHandle.set(urlHandle);
      // URLからハンドルが渡された場合、自動的に相関図を生成
