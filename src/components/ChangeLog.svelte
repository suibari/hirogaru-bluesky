<h5>v1.0.3</h5>
  <ul>
    <li>分析機能を<a href="https://blu-lyzer.vercel.app/" target="_blank">Blu-lyzer</a>に移管</li>
  </ul>
<h5>v1.0.2</h5>
  <ul>
    <li>ユーザカードの英語を修正</li>
    <li>Recent Moodの単語を調整し、意味のなさそうな言葉を弾くように設定</li>
  </ul>
<h5>v1.0.1</h5>
  <ul>
    <li>相関図生成時にローディングバーを表示</li>
    <li>活動タイムラインがずれていたバグを修正</li>
  </ul>
<h4>v1.0.0</h4>
  <ul>
    <li>データ分析機能追加(ユーザカードに最終活動時間、ぶるすこ歴、頻出ワード、活動タイムラインを表示)</li>
    <li>ハンドル名入力内容の保存タイミングを変更</li>
  </ul>
<h5>v0.5.7</h5>
  <ul>
    <li>シェア画像へのタイムスタンプ機能追加</li>
  </ul>
<h5>v0.5.6</h5>
  <ul>
    <li>ログイン機能追加</li>
  </ul>
<h5>v0.5.5</h5>
  <ul>
    <li>サジェスト検索機能追加</li>
  </ul>
<h5>v0.5.4</h5>
  <ul>
    <li>2回目実行時にデータが更新されないことがあるバグを修正</li>
  </ul>
<h5>v0.5.3</h5>
  <ul>
    <li>初回実行時にも関わらず異常に時間がかかっていたバグを修正</li>
  </ul>
<h5>v0.5.2</h5>
  <ul>
    <li>相関図中心の人から選択したユーザへのリプライ数、いいね数を表示する機能追加</li>
  </ul>
<h5>v0.5.1</h5>
  <ul>
    <li>データベースを更新できないバグ修正</li>
  </ul>
<h4>v0.5.0</h4>
  <ul>
    <li>相関図半径変更機能を追加</li>
    <li>高速化、取得データ量増加(バックエンドにDBを採用)</li>
    <li>関係分析機能を無効化</li>
    <li>関係分析無効化に伴いズーム機能有効化</li>
  </ul>
<h5>v0.4.4</h5>
  <ul>
    <li>プラットフォームをVercelに移行</li>
  </ul>
<h5>v0.4.3</h5>
  <ul>
    <li>リプライを取得できていなかったバグ修正</li>
  </ul>
<h5>v0.4.2</h5>
  <ul>
    <li>Appviewの修正に対応(feed.replyフィールドの有無でリプライかどうかを判定していたところを、適切な条件に修正)</li>
  </ul>
<h5>v0.4.1</h5>
  <ul>
    <li>シェアのテキストにURLも含めるよう修正</li>
  </ul>
<h4>v0.4.0</h4>
  <ul>
    <li>リファクタリング実施</li>
  </ul>
<h5>v0.3.3</h5>
  <ul>
    <li>相関図にフォローのみ（エンゲージメントのない）ユーザがいると表示できないバグを修正</li>
    <li>フォローのみユーザとの関係図矢印がおかしかったのを修正</li>
  </ul>
<h5>v0.3.2</h5>
  <ul>
    <li>フェッチ時のエラーでエラー要因が表示されなかったバグを修正</li>
    <li>ヘルプをローディングスピナーより手前に表示</li>
  </ul>
<h5>v0.3.1</h5>
  <ul>
    <li>関係性分析時に元の相関図に戻るボタンを表示</li>
    <li>関係性分析のゲージアニメーションを高速化</li>
  </ul>
<h4>v0.3.0</h4>
  <ul>
    <li>選択したユーザとの相互関係性の表示機能を追加</li>
  </ul>
<h5>v0.2.10</h5>
  <ul>
    <li>Googleアナリティクス追加</li>
  </ul>
<h5>v0.2.9</h5>
  <ul>
    <li>相関図の枠が満たない場合、フォローで埋めるよう修正</li>
    <li>バグ修正</li>
  </ul>
<h5>v0.2.8</h5>
  <ul>
    <li>相関図の枠が満たない場合、相互フォローで埋めるよう修正</li>
    <li>ハンドル名送信時にローカルストレージに保存し、次回アクセス時に読み込みする機能を追加</li>
    <li>リプライを10点に加点</li>
    <li>いいねの集計数が少なくなっていたバグ修正</li>
  </ul>
<h5>v0.2.7</h5>
  <ul>
    <li>アバター画像にアクセスできないユーザへの対応にエンバグがあったのを修正</li>
    <li>タップ時のハートが内部処理で消えず重くなっていたバグを修正</li>
  </ul>
<h5>v0.2.6</h5>
  <ul>
    <li>アイコンの大きさを一定値でクリッピングするよう修正</li>
    <li>その他軽微修正</li>
  </ul>
<h5>v0.2.5</h5>
  <ul>
    <li>集合体が苦手な人向けにデザインを全体的に配慮（アイコンの大きさを不均等化、相関図を回転させ左右非対称に、アイコンドラッグ可能に）</li>
    <li>バグ対応</li>
  </ul>
<h5>v0.2.4</h5>
  <ul>
    <li>生成画像のサイズを制限</li>
  </ul>
<h5>v0.2.3</h5>
  <ul>
    <li>ページアクセス時にタイトル表示</li>
  </ul>
<h5>v0.2.2</h5>
  <ul>
    <li>フォローバッジ復活</li>
    <li>生成画像にアプリ名を合成する機能追加</li>
    <li>ハートの大きさ調整</li>
  </ul>
<h5>v0.2.1</h5>
  <ul>
    <li>コミット漏れ</li>
  </ul>
<h4>v0.2.0</h4>
  <ul>
    <li>ノードタップ時にハートがエンゲージメントに応じて出現</li>
    <li>フォローバッジ削除</li>
    <li>バグ対応</li>
  </ul>
<h5>v0.1.3</h5>
  <ul>
    <li>フォローバッジ追加</li>
    <li>アバター画像にアクセスできないユーザに対応</li>
    <li>リプライを3点から5点に加点</li>
  </ul>
<h5>v0.1.2</h5>
  <ul>
    <li>テキストボックスとボタン配置のUIを改善</li>
  </ul>
<h5>v0.1.1</h5>
  <ul>
    <li>ハンドル名サジェスト機能を修正（@入れた場合に訂正を促す）</li>
  </ul>
<h4>v0.1.0</h4>
  <ul>
    <li>ファーストリリース</li>
  </ul>